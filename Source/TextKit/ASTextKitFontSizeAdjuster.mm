...
- (CGFloat)scaleFactor
{
  ...
  [_context performBlockWithLockedTextKitComponents:^(NSLayoutManager *layoutManager, NSTextStorage *textStorage, NSTextContainer *textContainer) {
    ...
    BOOL maxLinesFits = self->_attributes.maximumNumberOfLines > 0 ? NO : YES; // Weakify
    BOOL heightFits = isinf(self->_constrainedSize.height) ? YES : NO; // Weakify
    ...
    for (NSNumber *adjustedScaleObj in scaleFactors) {
      ...
      if (longestWordFits == NO) {
        ...
        longestWordFits = std::ceil((longestWordSize.width * adjustedScale)  <= self->_constrainedSize.width); // Weakify
      }
      ...
      if (longestWordFits == YES) {
        ...
        if (maxLinesFits == NO) {
          maxLinesFits = ([self lineCountForString:scaledString] <= self->_attributes.maximumNumberOfLines); // Weakify
        }
        ...
        if (maxLinesFits == YES && heightFits == NO) {
          ...
          heightFits = (stringSize.height <= self->_constrainedSize.height); // Weakify
        }
      }
    }
