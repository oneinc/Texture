...
- (void)truncate
{
  [_context performBlockWithLockedTextKitComponents:^(NSLayoutManager *layoutManager, NSTextStorage *textStorage, NSTextContainer *textContainer) {
    ...
    if (visibleCharacterRange.length < originalStringLength && self->_truncationAttributedString.length > 0) { // Weakify
      ...
      [textStorage replaceCharactersInRange:truncationReplacementRange
                       withAttributedString:self->_truncationAttributedString]; // Weakify
    }

    self->_visibleRanges = { visibleCharacterRange }; // Weakify
  }];
}
