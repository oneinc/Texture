...
- (void)setMaxConcurrentOperations:(NSUInteger)maxConcurrentOperations
{
  ...
  dispatch_async(_semaphoreQueue, ^{
    while (difference != 0) {
      if (difference > 0) {
        dispatch_semaphore_signal(self->_concurrentSemaphore); // Weakify
        difference--;
      } else {
        dispatch_semaphore_wait(self->_concurrentSemaphore, DISPATCH_TIME_FOREVER); // Weakify
        difference++;
      }
    }
  });
...
- (void)scheduleNextOperations:(BOOL)onlyCheckSerial
{
    ...
    if (_serialQueueBusy == NO) {
      ...
      if (operation) {
        ...
        dispatch_async(_serialQueue, ^{
          ...
          dispatch_group_leave(self->_group); // Weakify
          
          [self lock];
          self->_serialQueueBusy = NO; // Weakify
          [self unlock];
  ...
  dispatch_async(_semaphoreQueue, ^{
    ...
    if (operation) {
      dispatch_async(self->_concurrentQueue, ^{ // Weakify
        operation.block(operation.data);
        for (dispatch_block_t completion in operation.completions) {
          completion();
        }
